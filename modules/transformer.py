...
class TransformerEncoder(Module):
    ...
    def forward(self, src, mask=None, src_key_padding_mask=None, debug=False):
        ...
        for i, mod in enumerate(self.layers):
            ...
            if debug:
                attns.append(mod.attn)

        if self.norm is not None:
            output = self.norm(output)

        if debug:
            return output, torch.stack(attns, dim=0)
        else:
            return output


class TransformerEncoderCross(Module):
    r"""TransformerEncoder is a stack of N encoder layers
    ...
    def __init__(self, encoder_layer, num_layers, interpolation=None, norm=None):
        super(TransformerEncoderCross, self).__init__()
        self.layers = _get_clones(encoder_layer, num_layers)
        self.num_layers = num_layers
        self.norm = norm
    ...
    def forward(self, src, mask=None, src_key_padding_mask=None):
        ...
        for i, mod in enumerate(self.layers):
            if i % 2 == 1:
                ...
                output = mod(output)
                output = output.permute(2,1,0)
                if self.interpolation == 'down':
                    # TODO: rewrite down and up to save memory
                    output = output.permute(1,2,0)
                    output = F.max_pool1d(output, kernel_size=4, stride=4)
                    output = output.permute(2,0,1)
                elif self.interpolation == 'up':
                    output = output.permute(1,2,0)
...
class TransformerEncoderCross2(Module):
    r"""TransformerEncoder is a stack of N encoder layers
    ...
    def __init__(self, encoder_layer, num_layers, interpolation=None, norm=None):
        super(TransformerEncoderCross2, self).__init__()
...
class TransformerEncoderInterp(Module):
    r"""TransformerEncoder is a stack of N encoder layers
    ...
    def __init__(self, encoder_layer, num_layers, norm=None, scale=4, interpolation='down'):
        super(TransformerEncoderInterp, self).__init__()
        self.layers = _get_clones(encoder_layer, num_layers)
        self.num_layers = num_layers
        self.norm = norm
        self.scale = scale
        self.interpolation = interpolation
        assert self.interpolation in ['down', 'up']
    ...
    def forward(self, src, mask=None, src_key_padding_mask=None):
        ...
        for i, mod in enumerate(self.layers):
            ...
            if self.interpolation == 'down':
                # TODO: rewrite down and up to save memory
                output = output.permute(1,2,0)
                output = F.avg_pool1d(output, kernel_size=self.scale, stride=self.scale)
                output = output.permute(2,0,1)
            else:
                output = output.permute(1,2,0)
                output = F.interpolate(output, scale_factor=self.scale, mode='linear', align_corners=True)
                output = output.permute(2,0,1)
